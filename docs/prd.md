...
- Tapping a recent file reopens it immediately without requiring the file picker, provided the persisted permission is still valid. If the permission has expired (file deleted, provider removed), the entry is shown as unavailable.
- The recents list is the primary navigation surface. There is no folder browser.

**Share and print**

- The overflow menu includes "Share", which fires `ACTION_SEND` with the current file's `content://` URI (via `Intent.EXTRA_STREAM`) and resolved MIME type. This allows sharing the file itself via any registered app (email, messaging, Drive, etc.).
- If a document is unsaved (either a new document with no URI, or an existing document with unsaved changes), sharing is blocked and the user is prompted via an AlertDialog to save the document first. For existing files, the user can click "Save & Share" to save in-place and share immediately; for new files, the user can click "Save" to open the save picker. This ensures the recipient always receives a valid, up-to-date file and the warning is fully persistent.
...
- The find bar appears above the system keyboard, not as a modal dialog, and is styled using standard AOSP Material 3 colors (`surfaceContainer` background, `surface` container color, `8.dp` corner radius text fields).
- The find bar is designed in a toggleable compact mode: it starts in a single-row find-only layout and can be expanded to a double-row replace layout (utilizing equal-width text fields and Title Case "Replace" and "All" buttons) by tapping a left chevron indicator button.
- Find supports plain text matching. Case-sensitive toggle ("Aa") and match counts ("1/3") are embedded inside the query text box's trailing icon area to maximize editing space.
- Replace and Replace All actions are supported inside the expanded double-row layout.
- High-contrast search highlights are applied to the active editor matching ranges to satisfy WCAG contrast accessibility requirements.
...
- Default font: system default sans-serif (proportional font). User-selectable in settings between monospace and proportional.
- Default font size: 15sp. User-selectable: 12, 14, 15, 16, 18, 20, 24sp. Selection persists.
- Line spacing: user-configurable in settings. Four options: Compact (1.1×), Normal (1.3×), Relaxed (1.6×), Double (2.0×). Default is Normal. Selection persists.
...
- In landscape orientation on any device, the effective reading margin is the greater of the user's chosen value and 48dp, to avoid excessively long line lengths. On tablets and large-screen devices the window size class provides additional scaling (see build spec).
...
- Android 12/API 31 is the minimum supported version because dynamic color is a core product requirement, not an optional enhancement.
...
- The app does not crash on any Android version from API 31 (Android 12) onwards.

**Compatibility**

- Minimum SDK: API 31 (Android 12). Required so Material 3 dynamic color is available on every supported device.
- Target SDK: current stable (API 36 at time of writing).
- UI and layout: Standardized and consolidated exclusively on the standard AOSP (Material 3) design guidelines across all device vendors, removing vendor-specific visual branching.
- Tested on: stock Android (Pixel), Samsung One UI (operating with the consolidated AOSP styling), at least one mid-range device from another OEM, and at least one 10-inch Android tablet.
